...
ROOTFS="$ROOT/build/qemu-faisal-agent-security-m64"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys"
cp "$BUILD/agi_agent_security_m64_test" "$ROOTFS/bin/agi_agent_security_m64_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M64_DEVICE_NODE_MISSING
  echo FAISAL_M64_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M64_BOOT_OK
/bin/agi_agent_security_m64_test
rc=$?
echo FAISAL_M64_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M64_BOOT_OK' "$LOG"
grep -q 'M64_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M64_TEST_RC=0' "$LOG"
printf '%s\n' 'M64_QEMU_VALIDATION_OK'
