...
ROOTFS="$ROOT/build/qemu-faisal-experience-learning"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$BUILD/agi_experience_learning_test" "$ROOTFS/bin/agi_experience_learning_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M72_DEVICE_NODE_MISSING
  echo FAISAL_M72_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M72_BOOT_OK
/bin/agi_experience_learning_test
rc=$?
echo FAISAL_M72_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M72_BOOT_OK' "$LOG"
grep -q 'M72_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M72_TEST_RC=0' "$LOG"
printf '%s\n' 'M72_QEMU_VALIDATION_OK'
