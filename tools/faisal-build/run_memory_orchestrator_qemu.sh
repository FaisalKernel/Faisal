...
ROOTFS="$ROOT/build/qemu-faisal-memory-orchestrator"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$BUILD/agi_memory_orchestrator_test" "$ROOTFS/bin/agi_memory_orchestrator_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M82_DEVICE_NODE_MISSING
  echo FAISAL_M82_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M82_BOOT_OK
/bin/agi_memory_orchestrator_test
rc=$?
echo FAISAL_M82_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M82_BOOT_OK' "$LOG"
grep -q 'M82_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M82_TEST_RC=0' "$LOG"
printf '%s\n' 'M82_QEMU_VALIDATION_OK'
