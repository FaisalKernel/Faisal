...
ROOTFS="$ROOT/build/qemu-faisal-power-policy"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys"
cp "$BUILD/agi_power_policy_test" "$ROOTFS/bin/agi_power_policy_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M70_DEVICE_NODE_MISSING
  echo FAISAL_M70_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M70_BOOT_OK
/bin/agi_power_policy_test
rc=$?
echo FAISAL_M70_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M70_BOOT_OK' "$LOG"
grep -q 'M70_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M70_TEST_RC=0' "$LOG"
printf '%s\n' 'M70_QEMU_VALIDATION_OK'
