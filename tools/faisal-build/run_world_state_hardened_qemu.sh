...
ROOTFS=${FAISAL_WORLD_ROOTFS:-$ROOT/build/qemu-faisal-world-state-hardened}
LOG="$ROOTFS/qemu.log"
TEST="$BUILD/agi_world_state_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_WORLD_STATE_DEVICE_NODE_MISSING
  echo FAISAL_WORLD_STATE_RC=1
  poweroff -f
fi
echo FAISAL_WORLD_STATE_BOOT_OK
/bin/agi_world_state_test
rc=$?
echo FAISAL_WORLD_STATE_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
if [ "$QEMU_EXIT_ON_SUCCESS" = 1 ]; then
  ...
  while [ "$(date +%s)" -lt "$deadline" ]; do
    if grep -q 'FAISAL_WORLD_STATE_RC=0' "$LOG" 2>/dev/null; then
...
set -e
printf 'FAISAL_WORLD_STATE_QEMU_RC=%s\n' "$qemu_rc" >> "$LOG"
[ "$qemu_rc" -ne 124 ]
grep -q 'FAISAL_WORLD_STATE_BOOT_OK' "$LOG"
grep -q 'M73_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_WORLD_STATE_RC=0' "$LOG"
if grep -Eq 'BUG:|Oops:|Kernel panic|WARNING:.*kernel|general protection fault|KASAN:|UBSAN:|KCSAN:|rcu:.*stall|rcu_preempt.*stall|RCU_GP_WAIT_FQS|kthread starved|possible circular locking dependency|data-race|use-after-free|kernel BUG' "$LOG"; then
  echo 'FAISAL_WORLD_STATE_KERNEL_DIAGNOSTIC_FOUND' >&2
  exit 1
fi
echo FAISAL_WORLD_STATE_QEMU_VALIDATION_OK
