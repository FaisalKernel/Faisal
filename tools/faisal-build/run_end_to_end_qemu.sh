...
ROOTFS="$ROOT/build/qemu-faisal-end-to-end"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$BUILD/agi_end_to_end_test" "$ROOTFS/bin/agi_end_to_end_test"
...
grep -q 'FAISAL_M76_BOOT_OK' "$LOG"
grep -q 'M76_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M76_TEST_RC=0' "$LOG"
printf '%s\n' 'M76_QEMU_VALIDATION_OK'
