...
ROOTFS=${FAISAL_GRAPH_ROOTFS:-$ROOT/build/qemu-faisal-graph-telemetry}
LOG="$ROOTFS/qemu.log"
TEST="$BUILD/agi_graph_telemetry_test"
...
cc -O2 -Wall -Wextra -Werror -Wno-cpp -static \
  ...
  "$LINUX/tools/testing/selftests/agi_graph_telemetry_test.c" \
  -o "$TEST"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$TEST" "$ROOTFS/bin/agi_graph_telemetry_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_GRAPH_TELEMETRY_DEVICE_NODE_MISSING
  echo FAISAL_GRAPH_TELEMETRY_RC=1
  poweroff -f
fi
echo FAISAL_GRAPH_TELEMETRY_BOOT_OK
/bin/agi_graph_telemetry_test
rc=$?
echo FAISAL_GRAPH_TELEMETRY_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
if [ "$QEMU_EXIT_ON_SUCCESS" = 1 ]; then
  ...
  while [ "$(date +%s)" -lt "$deadline" ]; do
    if grep -q 'FAISAL_GRAPH_TELEMETRY_RC=0' "$LOG" 2>/dev/null; then
...
set -e
printf 'FAISAL_GRAPH_TELEMETRY_QEMU_RC=%s\n' "$qemu_rc" >> "$LOG"
[ "$qemu_rc" -ne 124 ]
grep -q 'FAISAL_GRAPH_TELEMETRY_BOOT_OK' "$LOG"
grep -q 'FAISAL_GRAPH_TELEMETRY_RC=0' "$LOG"
if grep -Eq 'BUG:|Oops:|Kernel panic|WARNING:.*kernel|general protection fault|KASAN:|UBSAN:|rcu:.*stall|rcu_preempt.*stall|RCU_GP_WAIT_FQS|kthread starved' "$LOG"; then
  echo 'FAISAL_GRAPH_TELEMETRY_DIAGNOSTIC_FOUND' >&2
  exit 1
fi
echo FAISAL_GRAPH_TELEMETRY_QEMU_VALIDATION_OK
