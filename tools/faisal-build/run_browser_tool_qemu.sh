...
ROOTFS="$ROOT/build/qemu-faisal-browser-tool"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$BUILD/agi_browser_tool_test" "$ROOTFS/bin/agi_browser_tool_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M75_DEVICE_NODE_MISSING
  echo FAISAL_M75_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M75_BOOT_OK
/bin/agi_browser_tool_test
rc=$?
echo FAISAL_M75_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M75_BOOT_OK' "$LOG"
grep -q 'M75_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M75_TEST_RC=0' "$LOG"
printf '%s\n' 'M75_QEMU_VALIDATION_OK'
