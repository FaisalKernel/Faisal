...
ROOTFS="$ROOT/build/qemu-faisal-verified-research"
LOG="$ROOTFS/qemu.log"

rm -rf "$ROOTFS"
mkdir -p "$ROOTFS/bin" "$ROOTFS/dev" "$ROOTFS/proc" "$ROOTFS/sys" "$ROOTFS/tmp"
cp "$BUILD/agi_verified_research_test" "$ROOTFS/bin/agi_verified_research_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M77_DEVICE_NODE_MISSING
  echo FAISAL_M77_TEST_RC=1
  poweroff -f
fi
echo FAISAL_M77_BOOT_OK
/bin/agi_verified_research_test
rc=$?
echo FAISAL_M77_TEST_RC=$rc
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
grep -q 'FAISAL_M77_BOOT_OK' "$LOG"
grep -q 'M77_SELFTEST_EXIT=0' "$LOG"
grep -q 'FAISAL_M77_TEST_RC=0' "$LOG"
printf '%s\n' 'M77_QEMU_VALIDATION_OK'
