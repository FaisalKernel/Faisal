...
OUT=${FAISAL_EVOLUTION_OUT:-$ROOT/build/frontier/evolution-validation}
SRC="$LINUX/tools/faisal-evolution/faisal_evolution.c"
INC="$LINUX/tools/faisal-evolution"
TEST="$LINUX/tools/testing/selftests/faisal_evolution_test.c"
BENCH="$LINUX/tools/testing/selftests/faisal_evolution_benchmark.c"
FUZZ="$LINUX/tools/testing/selftests/faisal_evolution_fuzz_test.c"
mkdir -p "$OUT"
cd "$LINUX"
...
printf 'FEV_EVOLUTION_ASAN_UBSAN_OK\n' | tee "$OUT/asan-ubsan.marker"
...
printf 'FEV_EVOLUTION_VALIDATION_OK\n' | tee "$OUT/validation.marker"
