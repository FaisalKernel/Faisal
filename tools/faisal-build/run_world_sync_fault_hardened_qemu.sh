...
ROOTFS=${FAISAL_WORLD_SYNC_ROOTFS:-$ROOT/build/qemu-faisal-world-sync-fault-hardened}
LOG="$ROOTFS/qemu.log"
TEST="$BUILD/agi_world_sync_fault_test"
...
if [ ! -e /dev/agi_lifecycle ]; then
  echo FAISAL_M198_DEVICE_NODE_MISSING
  echo M198_SELFTEST_RC=1
  echo FAISAL_M198_TEST_COMPLETE
  poweroff -f
fi
echo FAISAL_M198_BOOT_OK
/bin/agi_world_sync_fault_test
rc=$?
echo M198_SELFTEST_RC=$rc
echo FAISAL_M198_TEST_COMPLETE
poweroff -f
EOF
chmod +x "$ROOTFS/init"
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  if grep -q 'FAISAL_M198_TEST_COMPLETE' "$LOG" 2>/dev/null; then
...
set -e
printf 'FAISAL_M198_QEMU_RC=%s\n' "$qemu_rc" >> "$LOG"
[ "$qemu_rc" -ne 124 ]
grep -q 'FAISAL_M198_BOOT_OK' "$LOG"
grep -q 'M198_SUBSCRIPTION_REJECT_OK' "$LOG"
grep -q 'M198_SUBSCRIPTION_UPDATE_OK' "$LOG"
grep -q 'M198_WORLD_QUERY_OK' "$LOG"
grep -q 'M198_SYNC_WRITE_GUARD_OK' "$LOG"
grep -q 'M198_RESYNC_GUARD_OK' "$LOG"
grep -q 'M198_SELFTEST_EXIT=0' "$LOG"
grep -q 'M198_SELFTEST_RC=0' "$LOG"
grep -q 'FAISAL_M198_TEST_COMPLETE' "$LOG"
if grep -Eq 'BUG:|Oops:|Kernel panic|WARNING:.*kernel|general protection fault|KASAN:|KCSAN:|UBSAN:|rcu:.*stall|rcu_preempt.*stall|RCU_GP_WAIT_FQS|kthread starved|possible circular locking dependency|data-race|use-after-free|kernel BUG' "$LOG"; then
  echo FAISAL_M198_KERNEL_DIAGNOSTIC_FOUND >&2
  exit 1
fi
echo FAISAL_M198_QEMU_VALIDATION_OK
