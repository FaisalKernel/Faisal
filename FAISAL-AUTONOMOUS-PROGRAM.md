...
**Current tagged milestone:** `FAISAL-M96`
...
`FAISAL-M96` is validated at the implementation commit recorded in `FAISAL-PROGRAM-STATE.json`. M96 adds a bounded causal journal beside the M95 task journal, branch state transitions, M94 kernel intent-lease binding, resource/objective/lease admission gates, verified observation/result/verification evidence gating, explicit invalidation, digest-checked replay, and corruption fail-closed behavior. Its final verification includes strict build, host selftest, ASan/UBSan, TSan, real-kernel QEMU integration with `--require-kernel`, three smokes, full 23/23 audit, M95/M90/M91 regressions, security scan, benchmark evidence, and a tagged implementation commit. M96 does not claim arbitrary external-side-effect rollback, universal exactly-once semantics, model retraining, consciousness, performance improvement, or complete AGI.

The next selected dependency is **M97 capability-scoped tool registry with risk, cost, provenance, verification, revocation, and independent approval**. It must bind tool selection to explicit capabilities and M96 causal authority, fail closed on missing risk or provenance metadata, and keep model output non-authoritative.

## Governance

The program maintains small, reviewable commits and milestone tags. Every change records affected subsystems, compatibility impact, security impact, tests, benchmarks, and rollback. Unrelated untracked work is never silently included in a milestone. M73, M74, M75, M76, M77, M78, M79, M80, M81, M82, M83, M84, M85, M86, M87, M88, M89, M90, M91, M92, M93, M94, M95, and M96 evidence is stored under `tools/faisal-build/evidence/`, including machine-readable validation, QEMU, regression, benchmark, sanitizer, and build records. The active program state is updated after meaningful implementation, validation, failure, or dependency changes.
