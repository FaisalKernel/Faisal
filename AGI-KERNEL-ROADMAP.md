...
| Latest tagged FAISAL milestone | `FAISAL-M92`, userspace key-provider concurrency hardening |
| Kernel ABI | 37 |
...
| Next selected dependency | M93 multi-service provider registration, safe close, revocation broadcast, and crash recovery |
...
24. **M93 multi-service provider registration, safe close, revocation broadcast, and crash recovery:** next selected dependency; replace the bounded single-service pointer with a lifetime-safe registration contract while preserving private-key userspace isolation and M91 unsupported hardware gating.
