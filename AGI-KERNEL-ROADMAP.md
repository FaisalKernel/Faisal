...
| Latest tagged FAISAL milestone | `FAISAL-M90`, userspace signed-bundle key-provider contract |
| Kernel ABI | 37 |
...
| Next selected dependency | M91 provider-gated hardware key provisioning and attestation integration |
...
22. **M91 provider-gated hardware key provisioning and attestation integration:** next selected dependency; inspect real hardware or an explicitly identified provider first and report `UNSUPPORTED` when evidence is unavailable rather than fabricating hardware support.
