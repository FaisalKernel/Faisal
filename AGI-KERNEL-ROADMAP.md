...
| Latest tagged FAISAL milestone | `FAISAL-M95`, durable objective/task execution with replay-safe state and bounded recovery semantics |
| Kernel ABI | 38 |
...
| Next selected dependency | M96 supervisor-mediated provider ownership, stale-service reclamation, and explicit crash recovery |
| Complete-system status | Not operational; userspace services and end-to-end autonomous integration remain unfinished |

M94 establishes a temporal and causal authority boundary for trusted userspace brokers. The kernel does not evaluate natural language, model output, or semantic intent; it verifies a fixed digest and existing capability constraints. M95 adds a userspace durable objective/task state machine bound to a real FAISAL kernel session, with replay, idempotency, dependency, lease, retry, stop-condition, and corruption gates. No performance, world-first novelty, exactly-once side-effect, distributed-consensus, or complete-AGI claim is made.
...
27. **M96 supervisor-mediated provider ownership and explicit crash recovery:** next selected dependency. Define stale-service reclamation and restart semantics through an independent supervisor; do not infer recovery from process disappearance or add durable private-key storage without an identified provider.
