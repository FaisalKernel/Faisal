...
19. **M88 kernel Runtime Verification signal bridge:** next selected dependency; use the upstream `kernel/trace/rv` monitor/reactor architecture to provide an authenticated FAISAL signal source with explicit trace provenance, capability isolation, QEMU validation, security review, benchmark, and full regression evidence.
